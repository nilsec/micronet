...
def create_network(input_shape, name, run):
    ...
    # Get maxima
    gt_maxima = max_detection(tf.reshape(gt_soft_mask, list(output_shape) + [1]), [1,1,5,5,1], 0.5)
    pred_maxima = max_detection(tf.reshape(soft_mask, list(output_shape) + [1]), [1,1,5,5,1], 0.5)
