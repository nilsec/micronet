...
def create_network(input_shape, name, run):
    ...
    gt_maxima = max_detection(tf.reshape(gt_soft_mask,[1] + gt_soft_mask.get_shape().as_list() + [1]), [1,1,5,5,1], 0.5)
    pred_maxima = max_detection(tf.reshape(soft_mask,[1] + gt_soft_mask.get_shape().as_list() + [1]), [1,1,5,5,1], 0.5)
